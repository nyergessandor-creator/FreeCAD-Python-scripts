...
class TelescopingLeg:
    ...
    def set_extension(self, extension_mm):
        ...
        Args:
            extension_mm: Extension amount in mm (0-40)
        """
        extension_mm = max(0.0, min(40.0, extension_mm))  # Clamp to valid range
...
class RubiksCube:
    ...
    def _get_face_cubelets(self, face_name):
        """Get all cubelet objects for a given face by checking their current LOCAL grid positions."""
        face_info = self.faces[face_name]
        layer_test = face_info["layer"]
        
        # First, update all grid positions based on current placements
        self._update_grid_positions()
        
        cubelets = []
        
        # Now select by grid position
        for (x, y, z), corner in self.corner_cubelets.items():
            if layer_test(x, y, z):
                cubelets.append(corner.part_obj)
        
        for (x, y, z), edge in self.edge_cubelets.items():
            if layer_test(x, y, z):
                cubelets.append(edge)
        
        for (x, y, z), center in self.center_cubelets.items():
            if layer_test(x, y, z):
                cubelets.append(center)
        
        return cubelets
    
    def _update_grid_positions(self):
        """Update grid position dictionaries based on current cubelet placements relative to cube center."""
        # Get cube center (model's world position)
        if self.model_obj.Placement.Base.Length > 0.1:
            # Cube has been moved
            cube_center = self.model_obj.Placement.Base
        else:
            # Cube is at origin
            cube_center = App.Vector(0, 0, 0)
        ...
        # Process corners
        for old_pos, corner in list(self.corner_cubelets.items()):
            world_pos = corner.part_obj.getGlobalPlacement().Base
            local_pos = world_pos - cube_center
            
            # Snap to grid with proper rounding
            grid_x = int(round(local_pos.x / SPACING))
            grid_y = int(round(local_pos.y / SPACING))
            grid_z = int(round(local_pos.z / SPACING))
            
            # Clamp to valid range
            grid_x = max(-1, min(1, grid_x))
            grid_y = max(-1, min(1, grid_y))
            grid_z = max(-1, min(1, grid_z))
            
            new_pos = (grid_x, grid_y, grid_z)
            corner.grid_position = new_pos
            new_corners[new_pos] = corner
            
            # Update FreeCAD properties
            corner.part_obj.GridX = grid_x
            corner.part_obj.GridY = grid_y
            corner.part_obj.GridZ = grid_z
        
        # Process edges
        for old_pos, edge in list(self.edge_cubelets.items()):
            world_pos = edge.getGlobalPlacement().Base
            local_pos = world_pos - cube_center
        ...
        # Process centers
        for old_pos, center in list(self.center_cubelets.items()):
            world_pos = center.getGlobalPlacement().Base
            local_pos = world_pos - cube_center
        ...
        self.corner_cubelets = new_corners
        self.edge_cubelets = new_edges
        self.center_cubelets = new_centers
